...
`Graph` also has lightweight interoperability entry points for common external graph shapes. Homogeneous graphs can round-trip through PyG, NetworkX, in-memory edge lists, and CSV edge-list files via `Graph.from_*` / `graph.to_*` helpers under `vgl.compat`, while DGL compatibility continues to cover homogeneous, heterogeneous, temporal, and block-oriented flows.
