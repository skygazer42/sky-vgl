...
`Block` is the compact relation-local message-flow container returned by `to_block(...)` or `Graph.to_block(...)`. It wraps one bipartite `graph` plus explicit source/destination node id metadata through `src_n_id`, `dst_n_id`, and edge metadata through `edata["e_id"]`. This gives VGL a first-class bridge to DGL-style block workflows without forcing every sampler to change contracts. `NodeNeighborSampler(..., output_blocks=True)` materializes `NodeBatch.blocks` through the same container while keeping the existing sampled `graph` and `seed_index` contract; this now covers homogeneous node sampling plus relation-local heterogeneous node sampling when the supervised `node_type` has exactly one inbound relation, for both local and stitched coordinator-backed paths. `LinkNeighborSampler(..., output_blocks=True)` now does the same for `LinkPredictionBatch.blocks` while preserving the existing link supervision fields; this covers homogeneous link sampling plus relation-local heterogeneous link sampling, including the stitched coordinator-backed path. On heterogeneous graphs the emitted block list stays scoped to one supervised relation, so heterogeneous node types with zero or multiple inbound relations still fail clearly and one block-bearing batch still requires one `edge_type` schema.
...
For node sampling, one plan context can now carry one seed or a rank-1 seed collection. Materialization keeps the sampled graph shared per context, then expands the supervision side into the existing flat `NodeBatch.seed_index` contract so models and tasks do not need a separate multi-seed batch type. `NodeNeighborSampler(..., output_blocks=True)` also derives one block per hop from that sampled subgraph, ordered outer-to-inner, so block edges stay restricted to sampled fanout structure. This now covers homogeneous node sampling plus relation-local heterogeneous node sampling when the supervised `node_type` has exactly one inbound relation, for both local full-graph sampling and stitched shard-local sampling through a coordinator-backed feature source. Heterogeneous node types with zero or multiple inbound relations still keep the explicit sampled-subgraph-only path because one block list still represents one relation schema per layer.
