...
`vgl.sparse` is the low-level sparse execution layer. It provides `SparseTensor`, COO/CSR/CSC conversion helpers, transpose, additive reductions, row/column structural selection, sparse edge payloads shaped `(nnz, ...)`, and sparse ops such as scalar-weighted sparse-dense matmul, sampled dense-dense matmul via `sddmm(...)`, and edge-wise normalization via `edge_softmax(...)`.
