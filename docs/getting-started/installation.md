...
The CI matrix currently exercises Python 3.10–3.12 with PyTorch 2.4+ (CPU + CUDA builds), plus nightly/manual smoke coverage for optional extras.

- The main CI only installs the lightweight extras (`networkx`, `scipy`, `tensorboard`).
- `pyg` and `dgl` are exercised separately in `interop-smoke.yml`, not in the default push/PR test matrix.
- `--backend all` only works when both backend extras are installed and otherwise fails early with a missing-backend preflight error.

For DGL/PyG interop installs, run `python scripts/interop_smoke.py --backend dgl` or `python scripts/interop_smoke.py --backend pyg` for the extras you installed; use `--backend all` only when both are present. Follow the [support matrix](../support-matrix.md) to align your local setup with a verified combo.
...
When a DGL or PyG smoke run fails because the backend is missing, `scripts/interop_smoke.py` now echoes the matching extra install command in the error message. Unsupported combined backend checks should fail clearly rather than silently downgrade: if only one of `pyg` / `dgl` is installed, keep using the matching single-backend smoke command and avoid `--backend all`.
