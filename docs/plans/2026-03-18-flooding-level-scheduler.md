# Flooding Level Scheduler Plan

## Goal

Add a callback that linearly schedules `FloodingTask.level` across epochs without modifying trainer core behavior.

## Scope

- Add `FloodingLevelScheduler` callback in `vgl.engine.callbacks`
...
- Resolve the first task in the wrapper chain that exposes `level`
- Schedule `level` linearly from `start_value` to `end_value` between `start_epoch` and `end_epoch`
- Apply the scheduled value on fit start and after each epoch
- Restore the original level on fit end
