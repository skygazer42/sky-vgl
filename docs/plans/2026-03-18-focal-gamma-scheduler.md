# Focal Gamma Scheduler Plan

## Goal

Add a callback-based training strategy that schedules task-level `focal_gamma` over epochs for focal-loss tasks.

## Scope

- Add `FocalGammaScheduler` callback in `vgl.engine.callbacks`
...
- Apply the scheduled value before each epoch by updating `task.focal_gamma`
- Support wrapped tasks such as `RDropTask` by resolving `base_task`
- Restore the original task `focal_gamma` after `fit()`
