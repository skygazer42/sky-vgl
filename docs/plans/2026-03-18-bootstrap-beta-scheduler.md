# Bootstrap Beta Scheduler Plan

## Goal

Add a callback that linearly schedules `BootstrapTask.beta` across epochs without modifying trainer core behavior.

## Scope

- Add `BootstrapBetaScheduler` callback in `vgl.engine.callbacks`
...
- Resolve the first task in the wrapper chain that exposes `beta`
