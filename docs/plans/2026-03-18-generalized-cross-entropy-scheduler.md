# Generalized Cross Entropy Scheduler Plan

## Goal

Add a callback that linearly schedules `GeneralizedCrossEntropyTask.q` across epochs without modifying trainer core behavior.

## Scope

- Add `GeneralizedCrossEntropyScheduler` callback in `vgl.engine.callbacks`
...
- Resolve the first task in the wrapper chain that exposes `q`
- Schedule `q` linearly from `start_value` to `end_value` between `start_epoch` and `end_epoch`
- Apply the scheduled value on fit start and after each epoch
- Restore the original `q` value on fit end
