# Pos Weight Scheduler Plan

## Goal

Add a callback-based training strategy that schedules task-level `pos_weight` over epochs for link-prediction losses.

## Scope

- Add `PosWeightScheduler` callback in `vgl.engine.callbacks`
...
- Apply the scheduled value before each epoch by updating `task.pos_weight`
- Support wrapped tasks such as `RDropTask` by resolving `base_task`
- Restore the original task `pos_weight` after `fit()`
