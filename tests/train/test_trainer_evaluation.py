...
def test_trainer_restore_training_checkpoint_rejects_non_integer_history_epochs_before_mutating_model(
    tmp_path,
):
    checkpoint = tmp_path / "bad-history-epochs-type.pt"
    model = ToyModel()
    torch.save(
        {
            ARTIFACT_FORMAT_KEY: CHECKPOINT_FORMAT,
...
def test_trainer_restore_training_checkpoint_rejects_stop_reason_without_stopped_early_before_mutating_model(
    tmp_path,
):
    checkpoint = tmp_path / "bad-history-stop-reason.pt"
    model = ToyModel()
    torch.save(
        {
            ARTIFACT_FORMAT_KEY: CHECKPOINT_FORMAT,
...
def test_trainer_restore_training_checkpoint_rejects_non_string_history_monitor_before_mutating_model(
    tmp_path,
):
    checkpoint = tmp_path / "bad-history-monitor-type.pt"
    model = ToyModel()
    torch.save(
        {
            ARTIFACT_FORMAT_KEY: CHECKPOINT_FORMAT,
...
def test_trainer_restore_training_checkpoint_rejects_non_string_active_monitor_before_cross_check(
    tmp_path,
):
    checkpoint = tmp_path / "bad-active-monitor-type.pt"
    model = ToyModel()
    torch.save(
        {
            ARTIFACT_FORMAT_KEY: CHECKPOINT_FORMAT,
...
def test_trainer_restore_training_checkpoint_rejects_final_val_without_val_history_before_mutating_model(
    tmp_path,
):
    checkpoint = tmp_path / "bad-history-final-val-no-val-history.pt"
    model = ToyModel()
    torch.save(
        {
            ARTIFACT_FORMAT_KEY: CHECKPOINT_FORMAT,
...
def test_trainer_restore_training_checkpoint_rejects_final_train_mismatch_before_mutating_model(
    tmp_path,
):
    ...
    torch.save(
        {
            ...
            "history_state": {
                ...
                "monitor": "train_loss",
                "completed_epochs": 2,
                "train": [{"loss": 4.0}, {"loss": 3.0}],
                "epoch_elapsed_seconds": [0.1, 0.1],
                "final_train": {"loss": 2.0},
            },
            "trainer_state": {},
        },
        checkpoint,
...
def test_trainer_restore_training_checkpoint_rejects_final_val_mismatch_before_mutating_model(
    tmp_path,
):
...
def test_trainer_restore_training_checkpoint_rejects_val_monitor_without_full_val_history_before_mutating_model(
    tmp_path,
):
...
def test_trainer_restore_training_checkpoint_rejects_non_numeric_history_fit_elapsed_seconds_before_mutating_model(
    tmp_path,
):
    checkpoint = tmp_path / "bad-history-fit-elapsed-type.pt"
    model = ToyModel()
    torch.save(
        {
            ARTIFACT_FORMAT_KEY: CHECKPOINT_FORMAT,
...
def test_trainer_restore_training_checkpoint_rejects_non_numeric_fit_elapsed_seconds_before_mutating_model(
    tmp_path,
):
    checkpoint = tmp_path / "bad-fit-elapsed-type.pt"
    model = ToyModel()
    torch.save(
        {
            ARTIFACT_FORMAT_KEY: CHECKPOINT_FORMAT,
            ...
            "model_state_dict": {"weight": torch.tensor([9.0])},
            "metadata": {},
            "trainer_state": {"fit_elapsed_seconds": "bad"},
        },
        checkpoint,
    )
    trainer = Trainer(
...
def test_trainer_restore_training_checkpoint_rejects_non_numeric_fit_profile_total_before_mutating_model(
    tmp_path,
):
    checkpoint = tmp_path / "bad-fit-profile-total-type.pt"
    model = ToyModel()
    torch.save(
        {
            ARTIFACT_FORMAT_KEY: CHECKPOINT_FORMAT,
            ...
            "model_state_dict": {"weight": torch.tensor([9.0])},
            "metadata": {},
            "trainer_state": {"fit_profile": {"forward_seconds_total": "bad"}},
        },
        checkpoint,
    )
    trainer = Trainer(
...
def test_trainer_restore_training_checkpoint_rejects_non_integer_fit_profile_count_before_mutating_model(
    tmp_path,
):
    checkpoint = tmp_path / "bad-fit-profile-count-type.pt"
    model = ToyModel()
    torch.save(
        {
            ARTIFACT_FORMAT_KEY: CHECKPOINT_FORMAT,
            ...
            "model_state_dict": {"weight": torch.tensor([9.0])},
            "metadata": {},
            "trainer_state": {"fit_profile": {"train_step_count": "bad"}},
        },
        checkpoint,
    )
    trainer = Trainer(
