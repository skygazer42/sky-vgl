...
def test_graph_batch_from_samples_avoids_tensor_item_for_graph_labels(monkeypatch):
    ...
    def fail_item(self):
        raise AssertionError("GraphBatch graph labels should stay off tensor.item")
...
def test_graph_batch_from_samples_avoids_tensor_int_for_graph_labels(monkeypatch):
    graphs = [
        Graph.homo(
            edge_index=torch.tensor([[0], [1]]),
            x=torch.randn(2, 4),
    ...
    def fail_int(self):
        raise AssertionError("GraphBatch graph labels should stay off tensor.__int__")
