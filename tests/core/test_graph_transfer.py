...
def test_graph_to_moves_tensors_for_homo_hetero_and_temporal_graphs():
    ...
    moved_temporal = temporal.to(device="cpu")

    assert moved_homo is not homo
    assert moved_homo.schema == homo.schema
    assert moved_homo.x.device.type == "cpu"
    assert torch.equal(moved_homo.edata["edge_weight"], homo.edata["edge_weight"])
    ...
    assert torch.equal(
        moved_hetero.edges[("user", "follows", "user")].edge_attr,
        hetero.edges[("user", "follows", "user")].edge_attr,
    )
    assert (
    ...
    assert torch.equal(
        moved_temporal.edges[("node", "to", "node")].timestamp,
        temporal.edges[("node", "to", "node")].timestamp,
    )
