...
class RGATConv(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if heads < 1:
            raise ValueError("RGATConv requires heads >= 1")
        if dropout < 0.0 or dropout > 1.0:
            raise ValueError("RGATConv requires dropout to be in [0, 1]")
        ...
        self.relation_key_linears = nn.ModuleDict(
            {
                _relation_key(edge_type): nn.Linear(in_channels, hidden_channels, bias=False)
        ...
        self.relation_edge_linears = (
            nn.ModuleDict(
                {
                    _relation_key(edge_type): nn.Linear(edge_channels, hidden_channels, bias=False)
                    for edge_type in self.relation_types
                }
            )
    ...
    def forward(self, graph):
        ...
        for edge_type in self.relation_types:
            ...
            relation_keys = self.relation_key_linears[relation_name](graph.nodes[src_type].x).view(
                -1,
                self.heads,
                self.out_channels,
            )
            relation_values = self.relation_value_linears[relation_name](graph.nodes[src_type].x).view(
                -1,
                self.heads,
                self.out_channels,
            )

            if self.relation_edge_linears is not None:
                edge_attr = _resolve_relation_edge_attr(graph, edge_type, self.edge_channels, "RGATConv")
                edge_bias = self.relation_edge_linears[relation_name](edge_attr).view(-1, self.heads, self.out_channels)
                relation_keys = relation_keys + edge_bias
                relation_values = relation_values + edge_bias
