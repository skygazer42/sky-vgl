...
- `vgl.ops` centralizes reusable graph transforms plus graph queries. Alongside `find_edges(...)`, `edge_ids(...)`, and `has_edges_between(...)`, it now exposes `in_edges(...)`, `out_edges(...)`, `predecessors(...)`, and `successors(...)` for ordered one-hop adjacency plus `in_degrees(...)` / `out_degrees(...)` for DGL-style degree inspection. Degree queries return `int` for one node, tensors for many or all nodes, and continue to respect storage-backed declared node counts so isolated nodes surface as zero instead of disappearing. `in_subgraph(...)` / `out_subgraph(...)` keep node space intact, `reverse(...)` swaps endpoints while preserving `e_id` and storage-backed node-count context, and `to_block(...)` keeps relation-local message-flow rewrites and `src_n_id` / `dst_n_id` / `e_id` metadata aligned.
