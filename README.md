...
- `vgl.distributed` starts the shard-aware surface with partition manifests, deterministic local partition writing, local shard loading, shard/global id remapping, partition edge and adjacency queries, and single-process coordination contracts. The current local partition path now handles homogeneous, temporal homogeneous, and single-node-type multi-relation graphs without changing the manifest format, and partition graph queries can be scoped by `edge_type` when a shard contains multiple relations.
